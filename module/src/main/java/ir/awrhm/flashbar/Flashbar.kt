...
import ir.awrhm.flashbar.FlashbarContainerView
import ir.awrhm.flashbar.FlashbarView
import ir.awrhm.flashbar.R
import ir.awrhm.flashbar.anim.FlashAnim
import ir.awrhm.flashbar.anim.FlashAnimBarBuilder
...
//        flashbarView.adjustWitPositionAndOrientation(builder.activity, builder.gravity)
    ...
    private fun initializeContainerDecor() {
        with(flashbarContainerView) {
            ...
            builder.enterAnimBuilder?.let {
                setEnterAnim(it)
            }
            builder.exitAnimBuilder?.let {
                setExitAnim(it)
            }
    ...
    private fun initializeBarDecor() {
        with(flashbarView) {
            ...
            showIconScale(builder.iconScale)
...
//            setIconColorFilter(builder.iconColorFilter, builder.iconColorFilterMode)
    ...
    class Builder(internal var activity: Activity) {
        internal var gravity: Gravity = Gravity.BOTTOM
        ...
        private fun configureAnimation() {
            enterAnimBuilder = if (enterAnimBuilder == null) {
                val flashAnimBarBuilder = when (gravity) {
                    Gravity.TOP -> FlashAnim.with(activity).animateBar().enter().fromTop()
                    Gravity.BOTTOM -> FlashAnim.with(activity).animateBar().enter().fromBottom()
                }
                flashAnimBarBuilder
            } else {
                enterAnimBuilder?.let {
                    when (gravity) {
                        Gravity.TOP -> it.enter().fromTop()
                        Gravity.BOTTOM -> it.enter().fromBottom()
                    }
                }
            }
            ...
            exitAnimBuilder = if (exitAnimBuilder == null) {
                when (gravity) {
                    Gravity.TOP -> FlashAnim.with(activity).animateBar().exit().fromTop()
                    Gravity.BOTTOM -> FlashAnim.with(activity).animateBar().exit().fromBottom()
                }
            } else {
                exitAnimBuilder?.let {
                    when (gravity) {
                        Gravity.TOP -> it.exit().fromTop()
                        Gravity.BOTTOM -> it.exit().fromBottom()
                    }
                }
            }
        }
